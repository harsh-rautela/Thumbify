...
const StyleSelector = ({value,onChange,isOpen,setIsOpen}:{value:ThumbnailStyle; onChange:(style:ThumbnailStyle)=>void; isOpen:boolean; setIsOpen:(open:boolean)=>void}) => {
   const styleDescription: Record<ThumbnailStyle,string>={
    ...
    "Minimalist":"Clean, simple, lots of whiate space",
   ...
   const styleIcons: Record<ThumbnailStyle,React.ReactNode>={
    ...
    <div className='relative space-y-3 dark'>
        ...
        <button  className='flex w-full items-center justify-between rounded-md border 
        px-4 py-3 text-left transition bg-white/8 border-white/10 text-zinc-200 hover:bg-white/12' type='button' onClick={()=>setIsOpen(!isOpen)}>
