...
const Generate = () => {
  ...
  const handleGenerate = async ()=>{
    ...
    const response = await api.post('/api/thumbnail/generate',api_payload);
    if(response?.data?.thumbnail){
      navigate('/generate/'+ response?.data?.thumnail._id);
            ...
            <div className={`space-y-6 ${id && 'pointer-events-none'}`}>
              <div className="p-6 rounded-2xl bg-white/8 border border-white/12 shadow-xl space-y-6">
                  ...
                  <div className="space-y-5">
                      ...
                      <div className="sppace-y-2">
                          <label className="block text-sm font-medium">
                            Additional Prompts <span className="text-zinc-400 text-xs">
                              (optional)
                            </span>
                          </label>
                          <textarea value={additionalDetails} onChange={(e)=>setAdditionalDetails(e.target.value)} rows={3} placeholder="Add any Specific elements,mood pr style preferences"
                            className="w-full px-4 py-3 rounded-lg border border-white/10 text-zinc-100 placeholder:text-zinc-400 focus:outline-none focus:ring-2 focus:ring-pink-500 resiz-none"/>
                      </div>
                  </div>
                  {!id && (
